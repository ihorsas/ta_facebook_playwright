import Playwright, {Browser, BrowserContext, Page, selectors} from 'playwright';
import {attach, attachment, JasmineAllureReporter, log, step} from '../../config/jest-custom-reporter';
...
export abstract class Hooks extends JasmineAllureReporter {
  bcpObj: { browser: Browser, context: BrowserContext, page: Page };
  ...
  async before(): Promise<void> {
    ...
    this.bcpObj = await startBrowser("firefox", false);
    this.bcpObj.page.setDefaultTimeout(25 * 1000);
    this.bcpObj.page.setDefaultNavigationTimeout(25 * 1000);

    log('Before each');
  }

  async after(): Promise<void> {
    await closeBrowser(this.bcpObj);
